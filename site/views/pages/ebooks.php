...
<html>
<head>
	<title>Ebooks - GetValue</title>
</head>
<body>
	<section id="" style="margin-top:0px;">
		<div class="ebooks">
			<div class="container clearfix">
				<h2 class="col-md-12">EBOOKS</h2>
			    <div class="col-md-3">
			    		...
			    		<form method="get" class="row" action="">
			    			...
			    			<div class="form-group col-md-12">
			    				...
			    				<div class="input-group">
									<input type="number" class="form-control" placeholder="0" value="<?php echo $initialPrice; ?>" name="initialPrice">
    				...
    				<div id="shop" class="shop grid-container clearfix" data-layout="fitRows">
    					...
    					<div class="product_x clearfix">
    						...
    						<div class="product-desc">
    							...
    							<a class="add-to-cart btn-add btn-danger m-b-5" href="javascript:void();" onclick="buyProduct('<?php echo $product->id; ?>', '<?php echo $product->price; ?>', '<?php echo $product->name; ?>');">Pre-order Now <i id="progress<?php echo $product->id; ?>" class="progIc fa fa-spinner fa-spin"></i></a>
    							<?php } ?>
    						</div>
    					</div>
    					<?php } ?>
				    						...
				    						if($total_products>0){
				    							...
				    							for ($i=1; $i <= $pages; $i++) {
				    								...
				    								if($current_page == $i){
				    									...
				    									$pagination.='<li class="page-item"><a class="page-link" href="'.base_url('ebooks?brand='.$brand.'&initialPrice='.$initialPrice.'&finalPrice='.$finalPrice.'&time='.$time.'&from='.(($strt*$pg_limit)+1).'&to='.($i*$pg_limit)).'">'.$i.'</a></li>';
				    								}
				    								$strt++;
				    							}
				    							if($current_page!=1){
				    								$dis_p="";
				    								$prev_url=base_url('ebooks?brand='.$brand.'&initialPrice='.$initialPrice.'&finalPrice='.$finalPrice.'&time='.$time.'&from='.($start_page-$pg_limit).'&to='.($start_page-1));
				    							}
				    							if($current_page!=$p_count){
				    								$dis_n="";
				    								$next_url=base_url('ebooks?brand='.$brand.'&initialPrice='.$initialPrice.'&finalPrice='.$finalPrice.'&time='.$time.'&from='.($end_page+1).'&to='.($end_page+$pg_limit));
				    							}
				    						}
				    					?>
