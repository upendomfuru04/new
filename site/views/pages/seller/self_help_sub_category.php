            ...
            <div class="col-md-7">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h3 class="panel-title"> Self Help Sub-Category</h3>
                    </div>
                    <div class="panel-body p-40 row">
                        <div class="table-responsive">
                            <table id="dataTable" class="display" cellspacing="0" width="100%">
                                ...
                                <tbody>
                                    <?php $counter=0; foreach($subcategories as $item){ $counter++;?>
                                    <tr>
                                        <td><?php echo $counter; ?></td>
                                        <td><?php echo ucwords($item->category); ?></td>
                                        <td><?php echo ucwords($item->name); ?></td>
                                        <td>
                                            <a href="<?php echo base_url().'seller/blog/edit_self_help_sub_category/'.$item->id;?>" class="btn btn-xs btn-info"><i class="fa fa-edit"></i></a>
                                            <a href="javascript:void();" onclick="deleteData('<?php echo base_url()."seller/blog/delete_self_help_sub_category/".$item->id;?>');" class="btn btn-xs btn-danger"><i class="ti-trash"></i></a>
                                        </td>
                                    </tr>
                                    <?php } ?>
                                </tbody>
                            ...
                            <div class="row">
                                <div class="form-group col-md-12">
                                    ...
                                    <select class="form-control" name="category">
                                        ...
                                        <?php 
                                        if($update!=""){ $catg=$edit_rows['category'];}
                                        foreach($categories as $cat){ ?>
                                            <option value="<?php echo $cat->id; ?>" <?php if($update!="" && $catg==$cat->id){ echo 'selected';} ?>><?php echo $cat->name; ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                                <div class="form-group col-md-12">
                                    <label>Sub-Category Name:</label>
                                    <input type="text" name="name" class="form-control" placeholder="" value="<?php if($update!=""){ echo $edit_rows['name'];} ?>">
                                </div>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#saveBtn').click(function(){
           ...
           $.ajax({
                url: '<?php echo base_url(); ?>seller/blog/save_self_help_sub_categories',
                type: 'POST',
        ...
        $('#updateBtn').click(function(){
            ...
            $('.progressBarBtn').css('display','inline-block');
            $.ajax({
                url: '<?php echo base_url(); ?>seller/blog/update_self_help_sub_categories/<?php echo $update; ?>',
                type: 'POST',
                data:$('#data-form').serialize(),
                async: true,
                processData: false,
                success: function (data) {
                    if(data.trim()=='Success'){
                        ...
                        $('#resultMsg').html(output);
                        document.getElementById("data-form").reset();
                        window.location="<?php echo base_url(); ?>seller/blog/self_help_sub_category";
                    }else{
                        $('#resultMsg').html(data);
                    }
                    $('.turnOnProgress').css('display','inline-block');
