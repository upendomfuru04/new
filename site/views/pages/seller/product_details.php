    ...
    <div class="home-page">
        <div class="row p-t-20">
            <div class="col-md-11 col-md-offset-">
                <?php if(isset($table_rows) && sizeof($table_rows)>0){ ?>
                    ...
                    <div class="panel-body row p-40">
                        <div class="col-md-8">
                            <table class="table table-bordered tb-Verticle-sm">
                                ...
                                <tr><td><strong>Summary: </strong></strong></td><td><?php echo ucfirst($table_rows['summary']); ?></td></tr>
                        ...
                        <div class="col-md-4">
                            ...
                            <?php
                                if($table_rows['preview_type']=='image'){
                                    echo '<p class="text-center m-t-20">Preview Pic</p>';
                                    echo '<img src="'.base_url().'media/products/preview/'.$table_rows['preview'].'" class="img-thumbnail productImgD">';
                                }else{
                                ...
                                if($table_rows['virtual_link']!=""){
                                    ...
                                    if($table_rows['media_type']=='image'){
                                        echo '<p class="text-center m-t-20">Media Pic</p>';
                                        if(strpos($table_rows['media'], ';')!==FALSE){
                                            $mdia=explode(";", trim($table_rows['media']));
                                            for ($i=0; $i < sizeof($mdia); $i++) { 
                                                if($mdia[$i]!=""){
                                                    echo '<img src="'.base_url().'products/medias/'.trim($mdia[$i]).'" class="img-thumbnail productImgD">';
                                                }
                                            }
                                        }else{
                                            echo '<img src="'.base_url().'products/medias/'.$table_rows['media'].'" class="img-thumbnail productImgD">';
                                        }
                                    }else{
                                        if(strpos($table_rows['media'], ';')!==FALSE){
                                            $mdia=explode(";", trim($table_rows['media']));
                        ...
                        <div class="col-md-12">
                            <hr>
                            <a href="<?php echo base_url(); ?>seller/product/edit_product/<?php echo $table_rows['id'];?>" class="btn btn-sm btn-info"><i class="fa fa-edit"></i> Edit</a>
                            <a href="javascript:void(0);" onclick="deleteProduct('<?php echo base_url(); ?>seller/product/delete_product/<?php echo $table_rows['id'];?>');" class="btn btn-sm btn-danger"><i class="ti-trash"></i> Delete</a>
                        </div>
                    </div>
                </div>
                <?php } ?>
            ...
            $.ajax({
                ...
                success: function(data){
                    if(data.trim()=='Success'){
                        window.location="<?php echo base_url(); ?>seller/product/products";
                    }else{
                        alert(data);
                    }
                }
