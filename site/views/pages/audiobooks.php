...
<html>
<head>
	<title>Audiobooks - GetValue</title>
</head>
<body>
	<section id="" style="margin-top:0px;">
		<div class="ebooks">
			<div class="container clearfix">
				<h2 class="col-md-12">AUDIOBOOKS</h2>
			    <div class="col-md-3">
				    						...
				    						if($total_products>0){
				    							...
				    							for ($i=1; $i <= $pages; $i++) {
				    								...
				    								if($current_page == $i){
				    									...
				    									$pagination.='<li class="page-item"><a class="page-link" href="'.base_url('audiobooks?brand='.$brand.'&initialPrice='.$initialPrice.'&finalPrice='.$finalPrice.'&time='.$time.'&from='.(($strt*$pg_limit)+1).'&to='.($i*$pg_limit)).'">'.$i.'</a></li>';
				    								}
				    								$strt++;
				    							}
				    							if($current_page!=1){
				    								$dis_p="";
				    								$prev_url=base_url('audiobooks?brand='.$brand.'&initialPrice='.$initialPrice.'&finalPrice='.$finalPrice.'&time='.$time.'&from='.($start_page-$pg_limit).'&to='.($start_page-1));
				    							}
				    							if($current_page!=$p_count){
				    								$dis_n="";
				    								$next_url=base_url('audiobooks?brand='.$brand.'&initialPrice='.$initialPrice.'&finalPrice='.$finalPrice.'&time='.$time.'&from='.($end_page+1).'&to='.($end_page+$pg_limit));
				    							}
				    						}
				    					?>
