            ...
            <div class="col-md-12">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h3 class="panel-title"><i class="ti-user fa-fw"></i> System Admins</h3>
                    </div>
                            ...
                            <table id="dataTable" class="display" cellspacing="0" width="100%">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><img src="<?php echo base_url();?>media/admin_avatars/<?php echo $avatar; ?>" class="img-thumbnail productThumb" /></td>
                                        <td><?php echo ucwords($account->first_name.' '.$account->surname); ?></td>
                                        <td><?php echo ucwords($account->gender); ?></td>
                                        <td><?php echo $account->phone; ?></td>
                                        <td><?php echo $account->email; ?></td>
                                        <td><?php if($account->createdDate!="") echo date('d M, Y', $account->createdDate); ?></td>
                                        <td>
                                            <?php if($account->role!='1'){ ?>
                                            <?php if($account->status=='2'){ ?>
                                            <a href="javascript:void();" onclick="activateAccount('<?php echo base_url()."admin/account/activate_admin_account/".$account->id;?>', '<?php echo $account->first_name.' '.$account->surname;?>');" class="btn btn-xs btn-success" data-toggle="tooltip" title="Activate"><i class="fa fa-check"></i></a>
                                            <?php }else{ ?>
                                            <a href="javascript:void();" onclick="diactivateAccount('<?php echo base_url()."admin/account/diactivate_admin_account/".$account->id;?>', '<?php echo $account->first_name.' '.$account->surname;?>');" class="btn btn-xs btn-warning" data-toggle="tooltip" title="Diactivate"><i class="fa fa-ban"></i></a>
                                            <?php }?>
                                            <a href="javascript:void();" onclick="deleteAccount('<?php echo base_url()."admin/account/delete_admin_account/".$account->id;?>', '<?php echo $account->first_name.' '.$account->surname;?>');" class="btn btn-xs btn-danger"><i class="ti-trash"></i></a>
