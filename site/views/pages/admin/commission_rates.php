            ...
            <div class="col-md-12">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h3 class="panel-title"> Add/Update Commission Rates</h3>
                    </div>
                                ...
                                <tbody>
                                    <?php $counter=0; 
                                        foreach($table_rows as $item){ 
                                            $counter++;
                                            $status="";
                                            $action='<a href="'.base_url().'admin/commission/edit_commission_rates/'.$item->id.'" class="btn btn-xs btn-info m-r-10"><i class="fa fa-edit"></i></a>';
                                            if($item->status=='2'){
                                                $action.='<a href="javascript:void();" onclick="activateData(\''.base_url()."admin/commission/activate_commission_rate/".$item->id.'\');" class="btn btn-xs btn-success m-r-10" title="activate"><i class="fa fa-check"></i></a>';
                                                $status="Disabled";
                                            }elseif($item->status=='0'){
                                                $action.='<a href="javascript:void();" onclick="diactivateData(\''.base_url()."admin/commission/diactivate_commission_rate/".$item->id.'\');" class="btn btn-xs btn-warning m-r-10" title="diactivate"><i class="fa fa-ban"></i></a>';
                                                $status="Active";
                                            }
                                            $action.='<a href="javascript:void();" onclick="deleteData(\''.base_url()."admin/commission/delete_commission_rate/".$item->id.'\');" class="btn btn-xs btn-danger"><i class="ti-trash"></i></a>';
                                    ?>
                                    <tr>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#saveBtn').click(function(){
            ...
            $.ajax({
                url: '<?php echo base_url(); ?>admin/commission/save_commission_rate',
                type: 'POST',
        ...
        $('#updateBtn').click(function(){
            ...
            $('.progressBarBtn').css('display','inline-block');
            $.ajax({
                url: '<?php echo base_url(); ?>admin/commission/update_commission_rate/<?php echo $update; ?>',
                type: 'POST',
                data:$('#data-form').serialize(),
                async: true,
                processData: false,
                success: function (data) {
                    if(data.trim()=='Success'){
                        ...
                        $('#resultMsg').html(output);
                        document.getElementById("data-form").reset();
                        window.location="<?php echo base_url();?>admin/commission/commission_rates";
                    }else{
                        $('#resultMsg').html(data);
                    }
                    $('.turnOnProgress').css('display','inline-block');
