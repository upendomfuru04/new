<?php
	...
	class Sale_model extends CI_Model{
		...
		public function coupons($userID, $account_type){
			$result=array();
		...
		public function check_new_refund($userID){
			$res=0;
		...
		public function check_new_affiliate_refund($userID){
			$res=0;
			$this->db->select('tbl_refunds.id');
			$this->db->from('tbl_refunds');
			$this->db->join('tbl_cart', 'tbl_cart.orderID=tbl_refunds.orderID');
		...
		public function sales($userID){
			$result=array();
			$this->db->select('tbl_products.id, tbl_cart.id as cartID, product_url, item_id, name, image, tbl_cart.orderID, tbl_cart.price, tbl_products.quantity as avQtns, tbl_cart.quantity, tbl_cart.createdDate, first_name, surname, category');
		...
		public function affiliate_sales($userID){
			$result=array();
			...
			$this->db->join('tbl_products', 'tbl_products.id = item_id');
			$this->db->join('tbl_orders', 'tbl_orders.orderID = tbl_cart.orderID');
			// $this->db->join('tbl_commissions', 'tbl_commissions.product = tbl_cart.item_id AND tbl_commissions.orderID=tbl_orders.orderID');
			...
			$this->db->where('tbl_affiliate_urls.seller_id', $userID);
		   	// $this->db->where('tbl_cart.referral_url', 'tbl_affiliate_urls.referral_url');
		   	$this->db->where('tbl_affiliate_urls.status', '0');
		   	$this->db->where('tbl_cart.status', '0');
		   	$this->db->where('tbl_orders.status', '0');
		   	$this->db->where('tbl_commissions.status', '0');
		   	$this->db->where('tbl_orders.is_complete', '0');
		   	$this->db->order_by('tbl_cart.createdDate', 'DESC');
		...
		public function customer_refunds($userID){
			$result=array();
		...
		public function affiliate_customer_refunds($userID){
			$result=array();
			$this->db->select('tbl_refunds.id, tbl_refunds.orderID, amount, tbl_refunds.createdDate, customer_note, is_processed, commission, tbl_cart.quantity');
			$this->db->from('tbl_refunds');
			$this->db->join('tbl_cart', 'tbl_cart.orderID=tbl_refunds.orderID');
			$this->db->join('tbl_affiliate_urls', 'tbl_affiliate_urls.referral_url=tbl_cart.referral_url');
			$this->db->join('tbl_products', 'tbl_products.id=tbl_cart.item_id');
		...
		public function approve_customer_refund($refundID, $userID, $account_type){
			...
			$this->db->where('orderID', $orderID);
		   	...
		   	if($is_paid_order=='1'){
 				...
 				$send=$this->Selcom_model->push_disbursement($orderID, $refundID, "", $payment_type, $phone, $amount, "refund", $userID, $account_type);
 				if($send['msg']=='success'){
		...
		public function force_approve_customer_refund($refundID, $userID, $account_type){ die("Testing12: ".$refundID);
			$this->db->set('is_processed', '0');
			$this->db->set('refunder', $userID);
	   		$this->db->where('id', $refundID);
	   		$this->db->where('status', '0');
		...
		public function load_coupon_details($info, $userID){
		...
		public function delete_seller_coupon($infoID, $userID, $account_type){
    	...
    	public function save_shop_coupons($data, $account_type, $userID){
    		$createdDate=time();
