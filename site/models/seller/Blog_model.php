<?php
	...
	class Blog_model extends CI_Model{
		...
		public function my_posts($userID){
			$result=array();
			$this->db->select('tbl_blog_post.id, image, title, tbl_blog_category.category, is_approved, postedDate, views, url, denied_reason');
		...
		public function self_help(){
			$result=array();
			$this->db->select('id, image, title, content, postedDate, views, url');
		...
		public function load_post_detail($post, $userID, $account_type){
		...
		public function load_blog_post_details($post){
			$this->db->select('id, title, image, category, content, seller_type');
			$this->db->where('id', $post);
		   	$this->db->where('status', '0');
	   		$query = $this->db->get('tbl_blog_post');
		   	if($query->num_rows() > 0)
		...
		public function load_self_help_details($post, $userID){
    	...
    	public function update_blog_post($post, $data){
    		$createdDate=time();
			$title=sqlSafe($data['title']);
			$content=sqlSafe($data['content']);
			$summary=sqlSafe($data['summary']);
    		...
    		$set=array(
	            'title' => $title,
	            'summary' => htmlspecialchars($summary, ENT_QUOTES),
	            'category' => htmlspecialchars($content, ENT_QUOTES),
	            'content' => $content,
	            'url' => $url
	        );
    		if($postImage!=""){ $set['image']=$postImage;}
